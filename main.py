...
def train(model, device, train_loader, scheduler, optimizer, use_l1=False, lambda_l1=0.01):
    ...
    for batch_idx, (data, target) in enumerate(pbar):
        ...
        processed += len(data)
        lr_trend.append(scheduler.get_last_lr())
...
def fit_model(net, optimizer, device, NUM_EPOCHS,train_loader, test_loader, use_l1=False, scheduler=None, save_best=False):
    ...
    for epoch in range(1,NUM_EPOCHS+1):
        ...
        train_acc, train_loss, lr_hist = train(
            model=net, device=device, train_loader=train_loader, optimizer=optimizer, use_l1=use_l1, scheduler=scheduler
        )
        ...
        if save_best:
            if test_loss < min_val_loss:
                print(f'Valid loss reduced from {min_val_loss:.5f} --> {test_loss:.6f}.  Saving model to...{save_path}')
                save_model(net, epoch, optimizer, save_path)
                min_val_loss = test_loss
            else:
                print('Valid loss did not inprove')
